...
# 接口配置
OLD_API_URL = "http://llm-platform-cid.bdeastmoney.net/llm-platform-search-api/search/modelV2"
NEW_API_URL = "http://llm-platform-cid.bdeastmoney.net/llm-platform-search-api/search/modelV2"
...
def process_question(question: str) -> Optional[Tuple[List[Dict], int]]:
    """处理单个问题，执行n次旧接口和n次新接口调用，比较n方种组合"""
    # 执行n次旧接口和n次新接口调用
    old_results = []
    new_results = []
        ...
        for q in questions:
            try:
                ...
                if result:
                    ...
                    print(f"[进度B] 已处理: {total_processed}/{len(questions)} | 差异问题: {total_diffs_found} | 当前差异数: {total_diff}")
                else:
                    print(f"[进度B] 已处理: {total_processed}/{len(questions)} | 差异问题: {total_diffs_found} | 验证通过")
...
if __name__ == "__main__":
    main("b_query.csv", "id_diff_report_B.csv")
